...
#if true   // TODO(sankara) Resolve with one or the other
        if (type != nullptr && type->Name().CompareCStr(tsVariantType)) {
            VariantTypeRef variant = VariantType::New(_typeManager, BadType());
            type = variant;
        }
#else
        if (type->BitEncoding() == kEncoding_Variant) {
            VariantTypeRef variant = VariantType::New(_typeManager, BadType());
            type = variant;
        }
#endif
...
#if false
//------------------------------------------------------------
TypeRef TDViaParser::ParseVariantType()
{
    if (!_string.EatChar('('))
        return BadType();
    VariantTypeRef variantType = VariantType::New(_typeManager, _typeManager->BadType());
    if (!_string.EatChar(')'))
        return BadType();
    return variantType;
}
